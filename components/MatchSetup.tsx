...
import { Team, MatchFixture, TournamentFormat, Player } from '../types.ts';
...
type WizardStep = 'team_select' | 'squads' | 'toss' | 'openers';

export const MatchSetup: React.FC<MatchSetupProps> = ({ teams, onMatchReady, onCancel, onTeamUpdate, onCreateTeam }) => {
  ...
  const handleTeamSelect = (teamId: string) => {
    if (selectionStage === 'HOME') {
      ...
      setConfig(prev => ({ ...prev, teamBId: teamId }));
      // Transition to Squads
      // Auto-populate squads with first 11
    ...
    <div className="h-full flex flex-col bg-slate-50 relative overflow-hidden">
      ...
      {isCreatingTeam && (
        <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] p-8 w-full max-w-sm shadow-2xl animate-in zoom-in-95">
             <h3 className="text-xl font-black text-slate-900 mb-4">Add New Team</h3>
             <input 
               value={newTeamNameInput}
               onChange={e => setNewTeamNameInput(e.target.value)}
               placeholder="Team Name"
               autoFocus
               className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-bold text-slate-800 outline-none focus:ring-2 focus:ring-indigo-500 mb-6"
             />
      ...
      <div className="p-4 md:p-6 border-b border-slate-200 bg-white z-10 shrink-0 shadow-sm">
        <div className="flex justify-between items-center mb-4 max-w-6xl mx-auto w-full">
           <div>
             <h1 className="text-2xl font-black text-slate-900 tracking-tight">Match Setup</h1>
             <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest mt-0.5">
               ...
               {step === 'squads' && 'Confirm Starting XIs'}
               {step === 'toss' && 'The Toss'}
        ...
        <div className="h-1 w-full max-w-6xl mx-auto bg-slate-100 rounded-full overflow-hidden flex">
           <div className={`h-full bg-indigo-600 transition-all duration-500 ${step === 'team_select' ? 'w-1/4' : step === 'squads' ? 'w-2/4' : step === 'toss' ? 'w-3/4' : 'w-full'}`} />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6">
        <div className="max-w-6xl mx-auto h-full">
            ...
            <div className="animate-in fade-in duration-300">
                ...
                <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm mb-8">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="space-y-2">
                         ...
                         <select 
                           ...
                           onChange={(e) => {
                             ...
                             if(fmt === 'Test') defOvers = 90;
                             setConfig(prev => ({...prev, format: fmt, overs: defOvers}))
                           }}
                           className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-bold text-sm text-slate-900 outline-none focus:border-indigo-500 transition-colors"
                         >
                      ...
                      <div className="space-y-2">
                         <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Overs per Innings</label>
                         <input 
                           type="number" 
                           value={config.overs} 
                      ...
                      <div className="space-y-2">
                         <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">Squad Rules</label>
                         <div 
                           onClick={() => setConfig(prev => ({...prev, allowFlexibleSquad: !prev.allowFlexibleSquad}))}
                           className={`flex items-center justify-between bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 cursor-pointer transition-colors ${config.allowFlexibleSquad ? 'bg-indigo-50 border-indigo-200' : ''}`}
                         >
                            <div>
                               <div className={`font-bold text-xs ${config.allowFlexibleSquad ? 'text-indigo-900' : 'text-slate-900'}`}>Flexible Squads</div>
                               <div className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">{config.allowFlexibleSquad ? 'Allowed (>11 Players)' : 'Standard (11 Players)'}</div>
                            </div>
                            <div className={`w-10 h-6 rounded-full transition-all relative ${config.allowFlexibleSquad ? 'bg-indigo-600' : 'bg-slate-300'}`}>
                               <div className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-all shadow-sm ${config.allowFlexibleSquad ? 'left-5' : 'left-1'}`} />
                            </div>
                         </div>
                      </div>
                   </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
                    <button onClick={() => setIsCreatingTeam(true)} className="aspect-square rounded-2xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-slate-400 hover:border-indigo-400 hover:text-indigo-600 hover:bg-white transition-all group">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full animate-in slide-in-from-right-8 duration-300">
                {[selectedTeamA, selectedTeamB].map((team, idx) => {
                    const side = idx === 0 ? 'A' : 'B';
                    const squad = side === 'A' ? config.teamASquad : config.teamBSquad;
                    
                    return (
                    <div key={team.id} className="bg-white rounded-3xl border border-slate-200 flex flex-col h-full max-h-[60vh] lg:max-h-[calc(100vh-12rem)] shadow-sm overflow-hidden">
            ...
            <div className="flex flex-col items-center justify-center min-h-[50vh] animate-in zoom-in-95 duration-500">
                <div className="flex items-center justify-center gap-8 md:gap-20 mb-12">
                    <div className={`text-center transition-all ${config.tossWinnerId === selectedTeamA.id ? 'scale-110 opacity-100' : config.tossWinnerId ? 'opacity-40 scale-90' : 'opacity-100'}`}>
                        ...
                        <button onClick={() => setConfig(prev => ({...prev, tossWinnerId: selectedTeamA.id}))} className="mt-4 px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-full text-[10px] font-black uppercase tracking-widest transition-colors">Won Toss</button>
                    </div>
                    ...
                    <div className={`text-center transition-all ${config.tossWinnerId === selectedTeamB.id ? 'scale-110 opacity-100' : config.tossWinnerId ? 'opacity-40 scale-90' : 'opacity-100'}`}>
                        ...
                        <button onClick={() => setConfig(prev => ({...prev, tossWinnerId: selectedTeamB.id}))} className="mt-4 px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-full text-[10px] font-black uppercase tracking-widest transition-colors">Won Toss</button>
                    </div>
                </div>
                ...
                {config.tossWinnerId && (
                    <div className="bg-white p-6 rounded-3xl shadow-xl border border-slate-100 animate-in slide-in-from-bottom-8">
                        ...
                        <div className="flex gap-3">
                        <button onClick={() => setConfig(prev => ({...prev, tossDecision: 'Bat'}))} className={`px-8 py-3 rounded-xl font-black uppercase text-xs tracking-widest border transition-all ${config.tossDecision === 'Bat' ? 'bg-slate-900 text-white border-slate-900 shadow-lg' : 'bg-white text-slate-400 border-slate-200 hover:bg-slate-50'}`}>Bat First</button>
                        <button onClick={() => setConfig(prev => ({...prev, tossDecision: 'Bowl'}))} className={`px-8 py-3 rounded-xl font-black uppercase text-xs tracking-widest border transition-all ${config.tossDecision === 'Bowl' ? 'bg-slate-900 text-white border-slate-900 shadow-lg' : 'bg-white text-slate-400 border-slate-200 hover:bg-slate-50'}`}>Bowl First</button>
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full animate-in slide-in-from-right-8 duration-300">
                ...
                {/* BATTING OPENERS */}
                <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm flex flex-col h-full max-h-[60vh] lg:max-h-[calc(100vh-12rem)]">
                        ...
                        {getBattingTeam()!.players.filter(p => (getBattingTeam()!.id === selectedTeamA!.id ? config.teamASquad : config.teamBSquad).includes(p.id)).map(p => {
                            ...
                            return (
                                ...
                                onClick={() => {
                                    ...
                                    else if (!config.nonStrikerId && config.strikerId !== p.id) setConfig(prev => ({...prev, nonStrikerId: p.id}));
                                }}
                                className={`p-2 rounded-xl border text-left transition-all flex items-center gap-3 ${isSelected ? 'bg-slate-50 border-slate-200 opacity-60' : 'bg-white border-slate-100 hover:border-indigo-300'}`}
                                >
                    ...
                    <button onClick={() => setConfig(prev => ({...prev, strikerId: '', nonStrikerId: ''}))} className="mt-3 text-[9px] font-bold text-slate-400 uppercase tracking-widest hover:text-red-500 text-center transition-colors">Reset Selection</button>
                </div>
                ...
                <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm flex flex-col h-full max-h-[60vh] lg:max-h-[calc(100vh-12rem)]">
                    ...
                    <div className="mb-4 bg-emerald-50/50 p-3 rounded-2xl border border-emerald-100">
      ...
      {/* FOOTER ACTIONS */}
      <div className="p-4 md:p-6 bg-white border-t border-slate-200 flex justify-between z-10 shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
         {step === 'team_select' ? (
            ...
            <button 
               onClick={() => {
                  ...
                  if (step === 'openers') setStep('toss');
               }} 
               className="px-6 py-3 rounded-xl text-slate-500 font-black uppercase text-xs tracking-widest hover:bg-slate-50"
            >
               Back
            </button>
         )}
