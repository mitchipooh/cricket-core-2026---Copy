...
import { Tournament, MatchFixture, Organization, Team } from '../types.ts';
import { PointsTable } from '../competition/PointsTable.tsx';
...
type TrnTab = 'OVERVIEW' | 'GROUPS' | 'FIXTURES' | 'STANDINGS';
...
export const TournamentDashboard: React.FC<TournamentDashboardProps> = ({ 
  ...
  const standingsData = useMemo(() => {
    // Map existing MatchFixtures to CompletedMatch interface for the points table engine
    ...
    // In a multi-group tournament, we usually show group-specific standings. 
    // Here we return standings for the entire tournament.
    const allTeamsInTournament = tournament.groups.flatMap(g => g.teams);
    return buildPointsTable(completedMatches, allTeamsInTournament);
    ...
    <div className="animate-in slide-in-from-bottom-8 duration-500 pb-20">
       <div className="flex items-center gap-6 mb-10">
        ...
        <div className="flex gap-4 mb-10 overflow-x-auto pb-4 no-scrollbar">
          {(['OVERVIEW', 'GROUPS', 'FIXTURES', 'STANDINGS'] as TrnTab[]).map(tab => (
                            ...
                            <div className="space-y-2 relative z-10">
                                {group.teams.length === 0 ? (
                                    ...
                                    group.teams.map(team => (
                                        <div key={team.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
        ...
        {activeTab === 'FIXTURES' && (
           <div className="space-y-4">
              <div className="flex justify-between items-center mb-6">
                 <h3 className="text-xl font-black text-slate-900">Season Schedule</h3>
                 <button onClick={onGenerateFixtures} className="bg-indigo-600 text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg hover:bg-indigo-500 transition-all">Auto Generate Round Robin</button>
              </div>
              {activeFixtures.map(f => (
                 <div key={f.id} className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm flex justify-between items-center">
                    <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-8">
                      <div className="flex items-center gap-4">
                        <span className="font-black text-slate-900">{f.teamAName}</span>
                        <span className="text-xs font-bold text-slate-300">VS</span>
                        <span className="font-black text-slate-900">{f.teamBName}</span>
                      </div>
                      {f.result && (
                    ...
                    <button onClick={() => onStartMatch(f)} className="px-4 py-2 bg-slate-50 border border-slate-200 text-slate-900 text-[10px] font-black uppercase rounded-lg">Match Center</button>
                 </div>
              ))}
              {activeFixtures.length === 0 && <div className="py-20 text-center bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 text-slate-400 font-bold uppercase text-xs">No fixtures scheduled yet</div>}
           </div>
        )}

        {activeTab === 'STANDINGS' && (
           <div className="animate-in slide-in-from-right-8">
              <h3 className="text-xl font-black text-slate-900 mb-6 px-4">Official Standings</h3>
              <PointsTable rows={standingsData} />
           </div>
        )}
